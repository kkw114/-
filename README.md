...
- **新增**：折叠样式三选一 — 极简灰线 `light` / 经典警告 `classic` / 完全隐藏 `none`
- **新增**：请求内容控制 — 可独立开关附带用户名/用户ID/视频简介，默认关闭以节省 Token
- **新增**：自定义判定维度编辑区，精细控制 AI 判断标准
- **美化**：手动拉黑按钮改为右侧浮动圆角胶囊，hover 交互动效
- **修复**：切换排序后拉黑按钮消失的问题
- **清理**：移除 `logger.ts` 死代码

### v0.2.0

- 全新设置面板 UI（渐变头部 + Tab 切换）
- IndexedDB 黑名单持久化，支持手动词条移除
- Token 消耗统计与费用估算
- 自定义计费单价
- Shadow DOM 穿透诊断工具 (`__ruozhi_diag()`)
